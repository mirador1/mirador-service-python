...
# since the last `stable-py-v*` tag.
#
# Mirror of the Java side's changelog.sh, adapted for Python's tag prefix
# (`stable-py-vX.Y.Z` not `stable-vX.Y.Z` to keep the two repos' tag
# spaces separate even when they share a runner / mirror).
#
# Categorisation follows Conventional Commits :
#   feat:     → ✨ Features
#   fix:      → 🐛 Bug fixes
#   perf:     → ⚡ Performance
...
#   bin/ship/changelog.sh --since v0.6.3  # custom range start
...
#   git tag -a stable-py-vX.Y.Z -m "..."
#   git push origin stable-py-vX.Y.Z
# =============================================================================
set -euo pipefail
...
# Default range : since the latest stable-py-v* tag.
if [ -z "$SINCE" ]; then
  SINCE=$(git tag -l "stable-py-v*" | sort -V | tail -1)
  if [ -z "$SINCE" ]; then
    echo "❌ No stable-py-v* tag found ; pass --since <ref>" >&2
    exit 1
  fi
fi
...
classify() {
  ...
  case "$type" in
    ...
    # `|| true` shields against `set -e` killing the caller when grep returns 1.
